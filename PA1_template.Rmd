...
```{r}
averagestepsperday <- tapply(activity$steps, activity$date, mean)
plot(averagestepsperday, type="l", xlab="Time (Days)", ylab="Average Number of Steps", main="Time Series of Average Number of Steps")
```

5-minute interval that contains the maximum number of steps
```{r}
intervalstepsmax <- tapply(activity$steps, activity$interval, max)
plot(intervalstepsmax, type="l", xlab="5 Minute Intervals", ylab="Maximum Number of Steps", main="Time Series of Maximum Number of Steps")
```
...
hist(stepsimputed, breaks = 30, main="Histogram of Total Steps per Day")
```
